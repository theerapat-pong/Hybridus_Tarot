...
function Calendar({
  ...
  const locale = localeProp === 'th' ? th : enUS;
  const isTh = localeProp === 'th';
    ...
    <DayPicker
      ...
      classNames={{
        ...
        nav_button: cn(
          ...
          "h-7 w-7 bg-transparent p-0 opacity-50 hover:opacity-100 hover:bg-primary/20 hover:text-primary"
        ),
        ...
        Dropdown: ({ value, onChange, children, ...props }: DropdownProps) => {
          ...
          return (
            <select
              ...
              className={cn(
                "bg-transparent text-foreground rounded-md border border-input px-1 py-0.5",
                "rdp-dropdown"
              )}
              style={{
            ...
            >
              {options.map((option, id: number) => (
                ...
                >
                  {isTh && props.name === "years" 
                    ? parseInt(option.props.children as string) + 543 
